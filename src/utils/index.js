 ...
 * @returns 
 */
export function dateToChinese(dateStr) {
    if (dateStr.length < 10) {
        dateStr = String(dateStr + " " + "00:00:00");
    }
    dateStr = dateStr.replace("-", "/");
    let changeDate = new Date(dateStr);
    let year = changeDate.getFullYear() + '年';
    let month = changeDate.getMonth() + 1 + '月';
    let date = changeDate.getDate() + '日';
...
export function arrayDeDuplication(array){
    var temp = [];
    var index = [];
    var l = array.length;
    for(var i = 0; i < l; i++) {
 ...
 */
export const groupProjectArray = (array, subGroupLength) => {
    let index = 0;
    const newArray = [];
...
/**
 * @description 筛选多店铺的区块数据
 * @param {array} lists 需要筛选的数据
 * @returns {array}
 */
export const convertMultipleArea = lists => {
    const convertList = [];
    lists.forEach(item => {
        if (!item.PointID) return;

        if (typeof item.PointID === "string" && item.PointID.includes("||")) {
            convertList.push(...item.PointID.split("||").map((point, index) => ({
                ...item,
                AreaID: item.AreaID.split("||")[index],
                AreaName: item.AreaName.split("||")[index],
                PointID: item.PointID.split("||")[index],
                floor_id: item.floor_id.split("||")[index],
                floor_name: item.floor_name.split("||")[index]
            })))
        } else {
...
/**
 * @description 筛选在时间范围内的数据
 * @param {object} data 需要筛选的数据
 * @returns {boolean}
 */
export const compareTime = data => {
    const currentTime = ~~dayjs().format("HHmm");
    const startDateCompare = dayjs().isAfter(dayjs(data.StartDate)) || dayjs().isSame(dayjs(data.StartDate), "day");
    const overDateCompare = dayjs().isBefore(dayjs(data.OverDate)) || dayjs().isSame(dayjs(data.OverDate), "day");
    return currentTime >= ~~data.playplan[0].StartTime.replace(":", "") && currentTime <= ~~data.playplan[0].EndTime.replace(":", "") && startDateCompare && overDateCompare;
}

/**
 * @description 获取当前地图数据JSON信息
 * @returns {object}
 */
export const getMapConfigJSon = callback => {
    const request = new XMLHttpRequest();
    request.open("get", "static/config.json");/*设置请求方法与路径*/
    request.send(null);/*不发送数据到服务器*/
    request.onload = function () {/*XHR对象获取到返回信息后执行*/
        if (request.status == 200) {/*返回状态为200，即为数据获取成功*/
            window._MAP_CONGIG_ = JSON.parse(request.responseText);
            window.MAP_CONFIG.POSITION_NODEID = window._MAP_CONGIG_.POSITION_NODEID;
            window.MAP_CONFIG.POSITION_FLOOR = window._MAP_CONGIG_.POSITION_FLOOR;
            callback()
        }
    }
}
...
/**
 * @description 获取当前地图数据信息
 * @returns {object}
 */
// export const getMapInfomation = () => ({
//     floorID: map.getCurFloorIDs()[0],
//     azimuthalAngle: map.getAzimuthalAngle(),
//     position: map.getMapPosition(),
//     polarAngle: map.getPolarAngle(),
//     cameraRadius: map.cameraRadius,
// })

/**
 * @description 判断是win10还是win7环境
 * @returns {boolean}
 */
export const judgementWinSystem = () => {
    // 判断
    const version = navigator.userAgent;
    let isWin10;
    if (version.includes("Windows NT 10.0") || version.includes("Windows NT 6.2")) {
        isWin10 = true;
    } else if (version.includes("Windows NT 6.1")) {
        isWin10 = false;
    }
    return isWin10;
}
/**
 * @description 获取UUID 随机数字
 * @returns {string}
 */
 export const getUUID = () => {
    const s = [];
    const hexDigits = "0123456789abcdefghigklmnopqrstuvxwyz";
    for (let i = 0; i < 36; i++) {
...
function Decrypt(s, k) {
    var r = '';
    r = des(k, HexToString(s), 0, 0);
    return r;
}
/// 加密测试函数
/// s     待加密的字符串
/// k     密钥
function DES_Encrypt(s) {

    var r = '';
    r = Encrypt(s, 'Shanewoo');
    return r;
}
/// 解密测试函数
/// s     待解密的字符串
/// k     密钥
function DES_Decrypt(s) {

    var r = '';
    r = Decrypt(s, 'Shanewoo');
    return r;
}
 ...
 * 授权码解密
 */
export const DecryptForAuthorizationCode = Decrypt